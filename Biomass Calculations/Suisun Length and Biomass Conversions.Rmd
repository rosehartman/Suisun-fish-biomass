...
d.fx3.unique <- unique(d.fx3[species != "", .(species, common, convert.tl2sl.fx3, convert.tl2fl.fx3)][order(common)])
d.fx3.unique[, convert.sl2tl.fx3 := 1/convert.tl2sl.fx3]
rm(d.fx3)
```
...
d.perry.unique <- unique(d.perry[, .(common, lengthtype = biomassconvert.lengthtype)][order(common)])
...
paste(str(d.fx3.unique), str(d.kim))
...
d.perry.unique.wide <- spread(d.perry.unique, key = lengthtype, value = dp)
 
setnames(d.perry.unique.wide, 
         old = colnames(d.perry.unique.wide),
         new = c(
           "common",
           "no.lengthtype",
           "dp.fl",
...
d.convertcheck.merge1 <- merge(d.fx3.unique[, .(common, dfx3)], d.kim[, .(common, dk)], by = c("common"), all = TRUE)
d.convertcheck.merge2 <- merge(d.sch[, .(common, dsch)], by = c("common"), all = TRUE)
d.convertcheck.merge3 <- merge(d.perry.unique[, .(common, dp)], by = c("common"), all = TRUE)

d.biomass.convert.merge2 <- merge(suisun.catch.species, d.biomass.convert.merge, by = "common", all = TRUE)

##############OLD DIAGNOSTIC TABLE #################
## join conversion table data to species list from suisun to see what we have and what we need
#d.biomass.convert.merge <- merge(d.fx3.unique[, .(common, dfx3)], d.kim, by = c("common"), all = TRUE)
#d.biomass.convert.merge2 <- merge(suisun.catch.species, d.biomass.convert.merge, by = "common", all = TRUE)
####################################################

# reorder columns in above two tables for easier diagnostic viewing
d.biomass.merge <- setcolorder(d.biomass.convert.merge, c("common", "dfx3", "dk", "species.x", "species.y", "convert.tl.sl", "convert.tl2sl.kim", "convert.tl.fl", "convert.fl2tl.kim", "convert.biomass.slope", "convert.biomass.exponent", "biomassconvert.lengthtype", "reference"))

d.biomass.merge2 <- setcolorder(d.biomass.convert.merge2, c("common", "ds", "dfx3", "dk", "species.x", "species.y", "convert.tl.sl", "convert.tl2sl.kim", "convert.tl.fl", "convert.fl2tl.kim", "convert.biomass.slope", "convert.biomass.exponent", "biomassconvert.lengthtype", "reference"))
...
### DIAGNOSTIC ZONE ###
# str(d.suisun)
d.biomass.merge2[, .(common, convert.tl.sl, convert.tl.fl, convert.tl2sl.kim, convert.fl2tl.kim)]
