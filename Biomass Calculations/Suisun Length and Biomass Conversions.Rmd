...
d.fx3.unique <- unique(d.fx3[species != "", .(species, common, convert.tl.sl, convert.tl.fl)][order(common)]) 
```
...
#  (note only Kimmerer has biomass conversions)
paste(str(d.fx3.unique), str(d.kimmerer))

# Create column to track conversion table species through merge processes (i.e. species in each table)
d.fx3.unique[, dfx3 := "T"]
d.kimmerer[, dk := "T"]

# join conversion table data to species list from suisun to see what we have and what we need
d.biomass.convert.merge <- merge(d.fx3.unique, d.kimmerer, by = c("common"), all = TRUE)
d.biomass.convert.merge2 <- merge(suisun.catch.species, d.biomass.convert.merge, by = "common", all = TRUE)

# reorder columns in above two tables for easier diagnostic viewing
d.biomass.merge <- setcolorder(d.biomass.convert.merge, c("common", "dfx3", "dk", "species.x", "species.y", "convert.tl.sl", "convert.tl2sl.Kimmerer", "convert.tl.fl", "convert.fl2tl.Kimmerer", "convert.biomass.slope", "convert.biomass.exponent", "biomassconvert.lengthtype", "reference"))

d.biomass.merge2 <- setcolorder(d.biomass.convert.merge2, c("common", "ds", "dfx3", "dk", "species.x", "species.y", "convert.tl.sl", "convert.tl2sl.Kimmerer", "convert.tl.fl", "convert.fl2tl.Kimmerer", "convert.biomass.slope", "convert.biomass.exponent", "biomassconvert.lengthtype", "reference"))

# above tables reordered for diagnostic viewing
# d.biomass.convert.merge[order(common)]
# d.biomass.convert.merge2[order(common)]
...
d.missing.conversion <- d.biomass.convert.merge2[ds == "T" & is.na(dk), ][order(common)]

# Total catch number per species
# str(d.suisun)
d.suisun.summary <- d.suisun[, .(sum.catch.perspecies = sum(Count)), by = CommonName][order(-sum.catch)]
...
p <- ggplot(d.suisun.summary, aes(sum.catch))
p + geom_col() +        # , x, y, alpha, color, fill, group,linetype, size
  theme(axis.text.x = element_text(angle = 90))
+ x label color by dk (T = blue, na = red)

ggsave("Histogram Total Catch per Species.tif", width = 16, height = 9)

p + geom_histogram(aes(), binwidth = 5) # x, y, alpha, color, fill, linetype, size, weight

# plot histogram of average and max species.proportion.persample, grouped by species (stats in plot)

ggsave("Histogram Max Proportianate Catch per Species.tif", width = 16, height = 9)

# plot violin plot of species.proportion.persample, grouped by species

ggsave("Violin Proportionate Catch per Species.tif", width = 16, height = 9)
