...
setcolorder(d.biomass.convert.merge, c("common", "dfx3", "dk", "species.x", "species.y", "convert.tl.sl", "convert.tl2sl.Kimmerer", "convert.tl.fl", "convert.fl2tl.Kimmerer", "convert.biomass.slope", "convert.biomass.exponent", "biomassconvert.lengthtype", "reference"))

setcolorder(d.biomass.convert.merge2, c("common", "ds", "dfx3", "dk", "species.x", "species.y", "convert.tl.sl", "convert.tl2sl.Kimmerer", "convert.tl.fl", "convert.fl2tl.Kimmerer", "convert.biomass.slope", "convert.biomass.exponent", "biomassconvert.lengthtype", "reference"))
...
```{r}
#####################################################################################
## NOW COMPARE MISSING WITH SPECIES OCCURRENCE TO SEE IF WE CARE ABOUT THE MISSING ##
#####################################################################################
...
# Total catch number and sample occurrence per species
str(d.suisun)
d.suisun.summary <- d.suisun[, .(sum.catch = sum(Count)), by = CommonName][order(-sum.catch)]

d <- ggplot(d.suisun.summary, aes(CommonName, sum.catch))
g + geom_col() +        # , x, y, alpha, color, fill, group,linetype, size
theme(axis.text.x = element_text(angle = 90))

g + geom_histogram(binwidth = 5) # x, y, alpha, color, fill, linetype, size, weight
